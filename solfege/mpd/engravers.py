...
from solfege.mpd import MusicalPitch, Rat
...
class TieEngraver(Engraver):
    ...
    def engrave(self, widget, gc, staff_yoffset):
        ...
        w = self.m_note2.m_xpos - self.m_note1.m_xpos - dim.xshift * 1.3
        h = 8
        widget.window.draw_arc(gc, False,
              int(self.m_note1.m_xpos + dim.xshift * 1.2),
              int(staff_yoffset + dim.linespacing*(self.m_note2.m_ypos-1)/2 + 3),
              int(w), h, 64 * 180, 64*180)
...
class NoteheadEngraver(Engraver):
    def __init__(self, shift, ypos, musicalpitch, duration):
        ...
        Engraver.__init__(self)
        if duration.m_nh < 2:
            self.m_head = const.NOTEHEAD_0
        elif duration.m_nh > 2:
            self.m_head = const.NOTEHEAD_2
        else:
            self.m_head = const.NOTEHEAD_1
        self.m_shift = shift
    ...
    def __str__(self):
        return "(NoteheadEngraver: xpos:%s, ypos:%i, head:%i, shift=%i)" % (self.m_xpos,
                                                                    self.m_ypos,
                                                                    self.m_head,
                                                                    self.m_shift)
    def __repr__(self):
        return "<NoteheadEngraver xpos:%s, ypos:%i, head:%i, shift=%i>" % (
                self.m_xpos, self.m_ypos, self.m_head, self.m_shift)
...
class TupletEngraver(Engraver):
    ...
    def engrave(self, widget, gc, staff_yoffset):
        self.do_layout()
        dim = dimentions[self.m_fontsize]
        m = self.m_stems[0].m_xpos + (self.m_stems[-1].m_xpos -self.m_stems[0].m_xpos) / 2
        x1 = m - 6
        x2 = m + 6
...
class StemEngraver(Engraver):
    ...
    def shift_noteheads(self):
        ...
        for idx, head in enumerate(heads):
            if last_ypos is not None and head.m_ypos == last_ypos + 1 and heads[idx - 1].m_shift == 0:
                head.m_shift = 1 if self.m_stemdir == const.UP else -1
            last_ypos = head.m_ypos
    def __str__(self):
        return "(StemEngraver)"
    def __repr__(self):
        return "<NoteheadEngraver xpos:%s>" % (
                self.m_xpos, )
...
class StaffContext(_StaffCommon):
    ...
    def create_notehead_engraver(self, clef, elems):
        """
        Return a two item
        1: Return a list of the engravers that draw the note heads
        2: The stem engraver
        Will return the engravers in the same order as the notes in elems.
        """
        noteheads = []
        for elem in elems:
            ...
            elem.m_duration))
        return (noteheads,
            StemEngraver(noteheads, elems[0].m_duration, elems.m_stemdir, False))
