        ...
        if isinstance(self.m_tdict[timepos]['elem'][0], Skip):
            ...
            if insert_mode:
                if max_free >= elem.m_duration.get_rat_value():
                    self.m_tdict[timepos + Rat(1, 1000000)] = self.m_tdict[timepos]
                    del self.m_tdict[timepos]
...
class Score(object):
    class ConcatException(Exception): pass
    class StaffCountException(ConcatException): pass
    class StaffTypeException(ConcatException): pass
    class VoiceCountException(ConcatException): pass
    def __init__(self):
        self.m_staffs = []
        self.m_bars = []
    def copy(self):
    ...
    def get_bar_at(self, timepos):
        ...
        """
        for idx, bar in enumerate(self.m_bars):
