...
class Duration:
    ...
    def new_from_string(string):
        ...
        if m.groups()[0].isdigit():
            num = int(m.groups()[0])
        else:
            num == 0
        if m.groups()[1] in (u'', u'.', u'..', u'...', u'....'):
            dots = len(m.groups()[1])
        else:
            dots = None
        if not m or not m.groups()[0].isdigit() or dots is None:
            raise Duration.BadStringException(string)
        return Duration(num, dots)
    def __cmp__(self, B):
        """
    ...
    def new_from_rat(rat):
        ...
        return d
    def clone(self):
        return Duration(self.m_nh, self.m_dots, self.m_tuplet)
    def __str__(self):
