...
lessonfile_functions = {
        ...
        'csound': (True, lambda parser, orc, sco: CSound(parser, orc, sco)),
...
# We need this modified dict to use when parsing data into the
# sqlite-database. This because we want those data to be in C-locale
# so that the program can be run in different locales from the same
# database.
lessonfile_functions_no_i18n = dict(lessonfile_functions)
lessonfile_functions_no_i18n['_'] = (False, lambda f: f)
lessonfile_functions_no_i18n['_i'] = (False, lambda f: f)
...
class MpdParsable(MusicBaseClass):
    ...
    def get_err_context(self, exception, lessonfile_ref):
        ...
        """
        if getattr(exception, 'm_obj_lineno', False) is not False:
    ...
    def complete_to_musicdata_coords(self, lessonfile_ref, exception):
        ...
        assert getattr(exception, '_coords_done', False) == False
...
class Rvoice(VoiceCommon):
    def get_mpd_music_string(self, lessonfile_ref):
        ...
        try:
            ...
            if lessonfile_ref.header.random_transpose[0]:
                return "\\staff\\transpose %s\\relative %s{ %s }" % (
                    lessonfile_ref.m_transpose.get_octave_notename(),
...
class CSound(MusicBaseClass):
    def __init__(self, parser, orc, sco):
...
class LessonfileCommon(object):
    ...
    def parse_file(self, filename):
        ...
        self.parse_string(open(uri_expand(filename), 'rU').read(), really_filename=filename)
...
class IdByNameLessonfile(QuestionsLessonfile, TestSupport):
    def parse_string(self, s, really_filename=None):
        ...
        if self.header.labelformat == 'progression':
            for question in self.m_questions:
                question.label = lessonfile_functions["progressionlabel"][1](question.name)
    ...
    class FrontPageCache(object):
        ...
        def parse_file(self, filename):
            ...
            try:
                ...
                self._data[filename] = {'title': str(exception).decode(sys.getfilesystemencoding(), 'replace'), 'mtime': None}
    def __init__(self):
        self._data = {}
        self._dir_mtime = {}
            ...
            for filename in os.listdir(dir):
                ...
                if fn not in self._data or file_mtime > self._data[fn]['mtime']:
                    try:
                        ...
                        pass
    def iter_user_files(self):
        """
        Put the data from the lesson files below ~/.solfege/exercises/
        in the cache and yield the filename.
        """
        logging.debug("iter_user_files()")
        for fn in glob.glob(os.path.join(filesystem.user_data(), u"exercises/*/*/*")):
...
class LessonFileManager:
    class UnknownLessonID(Exception):
        pass
    def __init__(self):
    ...
    def parse(self, debug):
        lessonpath = [os.path.join('exercises', 'standard', 'lesson-files'),
                      filesystem.user_lessonfiles()]
        if debug:
            lessonpath.append(os.path.join('exercises', 'standard', 'regression-lesson-files'))
        self.m_uiddb = {}
        for dir in lessonpath:
            ...
            for filename in v:
                ...
                # parse_into_uiddb will return None on error.
                if lesson_id:
                    yield lesson_id, self.m_uiddb[lesson_id]
    def parse_into_uiddb(self, filename):
                ...
                if not p.header['lesson_id'] in self.m_uiddb:
                    ...
                    # We do the simplest thing here, we keep the oldest
                    # file and discard info from other files with the
                    # same lesson_id. Users are in the release notes
                    # for 3.15.3 advised to solve lesson_id conflict before
                    # upgrading.
                    if n['mtime'] < self.m_uiddb[p.header['lesson_id']]['mtime']:
                        self.m_uiddb[p.header['lesson_id']] = n
                return p.header['lesson_id']
    def get(self, lesson_id, fieldname):
        if lesson_id not in self.m_uiddb:
            raise self.UnknownLessonID(lesson_id)
...
def require_mgr():
    global mgr
    if not mgr:
        mgr = LessonFileManager()
        # This causes the LessonFileManager to parse all lesson files
        list(mgr.parse(True))
...
def parse_lesson_file_header(filename):
    ...
    m = r.search(s)
    p = dataparser.Dataparser(predef, lessonfile_functions_no_i18n)
    p.m_location = os.path.split(filename)
