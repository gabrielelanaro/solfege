...
class ConfigWindow(gtk.Dialog, cfg.ConfigUtils):
        ...
        def acc_ff(renderer, path, accel_key, accel_mods, hw_key):
            ...
            for notename in notenames:
                if (notename != notenames[int(path)]
                    ...
                    break
            if not is_unique:
                gu.dialog_ok(_("The accelerator in use for the tone “%s”. You have to choose another key.") % solfege.mpd.MusicalPitch.new_from_notename(notename).get_user_notename(), parent=self, msgtype=gtk.MESSAGE_ERROR)
                return
        ...
        def acc_ff(renderer, path, accel_key, accel_mods, hw_key):
            ...
            for interval in intervals:
                if (interval != intervals[int(path)]
                    ...
                    break
            if not is_unique:
                gu.dialog_ok(_("The accelerator in use for “%s”. You have to choose another key.") % mpd.Interval.new_from_int(intervals.index(interval)).get_name(), parent=self, msgtype=gtk.MESSAGE_ERROR)
                return
    ...
    def popup_alsa_connection_list(self, widget):
        menu = gtk.Menu()
        for clientid, portid, clientname, portname, labeltext in soundcard.alsa_sequencer.get_connection_list():
